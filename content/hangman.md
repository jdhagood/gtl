...
# Getting input
Python provides us with a really nice way to get user input from the termial using the `input` function. 
...
I put my program in a file called `'hangman.py'` in the directory `'python_sketchbook'`. I set my working direcotry to `'python_sketchbook'` and then ran this command.

```
C:\Users\jdhag\python_sketchbook>python hangman.py
```

If you have Python on your PATH variable (like I reccomended in lecture 0), then this should run the `hangman.py` file in the terminal where you can provide input.
...
# Filtering Input
Any time you are designing a program that humans will use, you need to consider how they will interact with that program and if their interactions can lead to crashes or bugs. (Some people, like me, intentially try to do this, so watch out! SQL injections are a prime example of this if you are familiar with these attacks.)
...
def get_user_guess():
    ...
    return user_input
```
Notice how we define `is_valid_input` inside of `get_user_guess`. We call functions defined inside of other functions _"helper functions"_, because they help us to cleanly implement our main function. We can call `is_valid_input` anywhere inside of `get_user_guess`, but it will not be accessable outside of `get_user_guess`. This helps with organization and preventing us from accidentially redefining functions in other places. (and yes, you can have helper functions inside of helper functions inside of helper functions inside of... well, you get the point).
...
Lets again roll this up into a function that will help us later
```Python
...
It is easy to determine when the game is lost because `lives` will at 0. However, determing when the game is won is more tricky. We need to check that we have guessed every letter in a word. Thankfully we are already keepign track of our correct guesses, so lets make a function to determine if we won or not.

```python
def is_game_won(word, correct_user_guesses):
    for letter in word:
        if letter not in correct_user_guesses:
            return False
...
# Displaying our word
Hangman relies on the user being able to see where their guessed letters are in the word. We can make a function to display the current progress given `word` and `correct_user_guesses`

```python
def display_hangman_word(word, correct_user_guesses):
    hangman_word = ""
    for letter in word:
        if letter in correct_user_guesses:
            hangman_word += letter + " "
        else:
            hangman_word += "_ "
    print(hangman_word)
